...
export default function IncomeChart({ transactions }: IncomeChartProps) {
  ...
  const processData = () => {
    const incomeTransactions = transactions.filter(t => t.type === TransactionType.INCOME && t.effectiveDate);
    ...
    // Ordenar por data
    const sortedEntries = Object.entries(monthlyIncome).sort((a, b) => {
    ...
    return (
      <div className="flex items-center justify-center h-64">
        <p style={{ color: 'var(--color-text-secondary)' }}>
          Nenhum dado de entrada encontrado
        </p>
      </div>
    );
  }
