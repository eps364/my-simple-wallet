...
export default function ExpenseChart({ transactions }: ExpenseChartProps) {
  ...
  const processData = () => {
    const expenseTransactions = transactions.filter(t => t.type === TransactionType.EXPENSE && t.effectiveDate);
    ...
    // Ordenar por data
    const sortedEntries = Object.entries(monthlyExpense).sort((a, b) => {
    ...
    return (
      <div className="flex items-center justify-center h-64">
        <p style={{ color: 'var(--color-text-secondary)' }}>
          Nenhum dado de saída encontrado
        </p>
      </div>
    );
  }
